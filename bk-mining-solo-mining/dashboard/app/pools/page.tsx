...
async function loadSummary(def: PoolDef): Promise<PoolSummary> {
  ...
  return { def, poolStats, nodeInfo, blocks };
}
...
function PoolTile({ s }: { s: PoolSummary }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[11px] text-slate-500 border-t border-slate-800 pt-2 font-mono">
        stratum+tcp://pool.oneclickminer.com:{s.def.stratumPort}
      </div>
    </Link>
  );
}
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Tap a tile to open its detailed view.</p>
      </div>
